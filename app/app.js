...
(function () {
  ...
  function num(v, def = 0) {
    // RU-friendly parsing: supports both "1.23" and "1,23".
  ...
  function updateTankSectionVisibility() {
    ...
    // Table must stay visible in BOTH modes.
    // "Единый тип груза" changes columns (see buildTankRows), not visibility.
  }
  ...
  function renderSingleCargoControls() {
    ...
    // ===== Tarировка UI (premium) =====
    // FIX: t раньше был undefined (ReferenceError ловился try/catch → визуал не рендерился)
    try{
      if (window.TT_TARIROVKA) {
        const trailers = getAllTrailers();
        const t = trailers.find(x => x.id === app.selectedTrailerId) || trailers[0] || null;
        window.TT_TARIROVKA.render(t, app.trailerState, recalc);
      }
    }catch(e){}
  ...
  function selectTrailer(id) {
    ...
    renderCurrent();
    // ===== Tarировка visual sync (premium) =====
    try{
      if (window.TT_TARIROVKA) window.TT_TARIROVKA.syncAfterRecalc(app.trailerState);
    }catch(e){}
  ...
  function recalc() {
    ...
    // ===== Tarировка: sync after recalc (premium) =====
    // Важно: recalc может менять литры/тонны программно (не через input-событие),
    // поэтому визуал должен синхронизироваться явно.
    try{
      if (window.TT_TARIROVKA) window.TT_TARIROVKA.syncAfterRecalc(tstate);
    }catch(e){}
  ...
  function bind() {
    ...
    if (copyBriefBtn) {
      copyBriefBtn.addEventListener('click', async () => {
        const brief = $('brief');
        const text = (brief && typeof brief.value === 'string') ? brief.value : '';
        if (!text.trim()) { showToast('Бриф пустой', 'warn'); return; }
        try {
          if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            brief.setSelectionRange(0, 0);
          }
          showToast('Бриф скопирован', 'ok');
        } catch (e) {
